...
int main(){
    ...
    public:
        explicit SegTree(int n){
            rt = new Node;
            using build = tuple<Node*, int, int, int>; // u, l, r, sta
            stack<build> stk;
            stk.emplace(rt, 1, n, 0);
            while (!stk.empty()){
                auto [u, l, r, sta] = stk.top();
                stk.pop();
                switch(sta){
                    case 0:
                        u->l = l, u->r = r, u->val = (l == 1 ? 1 : 0), u->dep = (l == 1 ? 1 : 0);
                        if(l != r){
                            stk.emplace(u, l, r, 1);
                            u->ls = new Node, u->rs = new Node;
                            stk.emplace(u->ls, l, (l + r) >> 1, 0);
                            stk.emplace(u->rs, ((l + r) >> 1) + 1, r, 0);
                        }
                        break;
                    case 1:
                        update(u);
