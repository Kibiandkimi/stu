...
//    queue<int> line;
    ...
    for(int i = 1; i <= cnt; i++){
        ...
        while(p2<Q[i].l){
            ...
            ans = max(ans, (queryX(1, Q[i].il[p2])-((Q[i].il[p2]-Q[i].ih[p1]>1)?queryMin(1, Q[i].ih[p1]+1, Q[i].il[p2]-1):0)) + (((Q[i].ih[p1+1]-Q[i].il[p2]>1)?queryMax(1, Q[i].il[p2]+1, Q[i].ih[p1+1]-1):queryX(1, Q[i].il[p2]))-queryX(1, Q[i].il[p2])));
            p2++;
        }
        Ans[Q[i].id] = ans;
    }
